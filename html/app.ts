...
document.getElementById("launch-button").addEventListener('click', () => {
	...
	gameStates.push(I.initGame(123));
...
function makeBoard(gameState: I.GameState) {
	...
	return acc + `<div class="tile" style="grid-row: ` + (e.get('y') + 15) + `; grid-column: ` + (e.get('x') + 15) + `;">` + makeQuarter(e) + `</div>`;
    }, "");
}
...
function makeQuarter(quarter: B.Quarter) {
	...
	svg += `<line x1="2" y1="0" x2="2" y2="2" stroke="black" stroke-linecap="round" />`;
    if (quarter.get("road").get("west"))
	svg += `<line x1="2" y1="2" x2="4" y2="2" stroke="black" stroke-linecap="round" />`;
    if (quarter.get("road").get("south"))
	svg += `<line x1="2" y1="2" x2="2" y2="4" stroke="black" stroke-linecap="round" />`;
    if (quarter.get("road").get("east"))
	svg += `<line x1="0" y1="2" x2="2" y2="2" stroke="black" stroke-linecap="round" />`;
